...
###I also could not come up with any better way to do this. I even searched for ways to do it using markdown and nothing was helpful - C. Vega
```
...
#----- C. Vega Edit (I am editing based on the assumption that the way I did Challenge 5 was correct)
#I think you missed a step here of making the matrix intro a data frame. Instead of creating a new vector with matrix made into a dataframe you continued using the info directly from the matrix. Try this:

# >coerced<- data.frame(matrix))  

#When the instructions say "as a dataframe" you would take your "coerced" vector and do:

# >as.logical(coerced[,2]) 

# This could give you the same data from your matrix, but is more simplified since it is now in a data frame. You would do the same for the factor valued instruction set as well- C. Vega
coerced<-data.frame(matrix)
coerced[,2]<-as.logical(coerced[,2])
coerced[,3]<-as.factor(coerced[,3])
str(coerced)
#Ithink the two of these are the same? THe output at least is.
...
```
 ...
 I also had a weird issue with the Rmarkdown file in general. I kept having issues closing the chunks. At one point I had to compleatly remove one and then remake it. Not sure why?
        (That is weird, but you did a great job of setting it up! It looks beautifully put together. -C. Vega)
 
 ###5
 Im not sure on annotating code. I basically just used the challenges themself, but im not sure if it is enough information.
        (I just tried it as how I would put headings of notes like "Matrices and Arrays" or "Lists and 
        Dataframes" so that they will be easier to find when I come back to them- C. Vega)